...
type VersionLike = BibleVersion & { name?: string }

function getVersionDisplayTitle(version: VersionLike): string {
  return version.local_title || version.name || ''
}
              ...
              {filteredVersionsForDropdown.map(version => (
                ...
                >
                  <div className="flex items-center">
                    ...
                    />
                  <span className="ml-3 text-card-foreground">{version.abbreviation} - {getVersionDisplayTitle(version as VersionLike)}</span>
                  </div>
                  {version.has_audio && (
          ...
          {allVersionsToDisplay.map(version => (
            <div key={version!.bible_version_id} className="bg-card rounded-lg p-4 border border-border">
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm text-muted-foreground">{getVersionDisplayTitle(version as VersionLike)}</span>
              </div>
