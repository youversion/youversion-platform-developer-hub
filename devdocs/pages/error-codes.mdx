...
**Solution**: Ensure you're including a valid API key in the `X-App-Id` header with every request.
...
// Usage
try {
  const bibles = await makeApiRequest('https://api.youversion.com/v1/bibles', 'YOUR_API_KEY');
...
# Usage
bibles = make_api_request('https://api.youversion.com/v1/bibles', 'YOUR_API_KEY')
if bibles:
    print('Bibles:', bibles)
```
...
response=$(curl -s -w "%{http_code}" -H "X-App-Id: YOUR_API_KEY" \
...
1. **Invalid API key**: Use a fake API key to test 401 responses
